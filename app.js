...
//console.log("${getFollowerIds}');
const getFollowers ResultQuery= `select name from user where user_id in (${getFollowerIds});`; 
...
const getFollowingIds = getFollowingIdsArray.map((each Follower) => {
...
const following TweetIds = getTweetIdsArray.map((eachId) {
return eachId.tweet_id;
});
//console.log(following TweetIds);
//console.log(following TweetIds.includes (parseInt(tweetId)));
...
const getTweetIdsQuery = `select tweet_id from tweet where user_id in (${getFollowingIds});`;
const getTweetIdsArray = await database.all(getTweetIdsQuery);
const getTweetIds = getTweetIdsArray.map((each Tweet) => { 
...
const getUsernameReplyTweetsQuery= `select user.name, reply.reply from user inner join reply on user.user_id=reply.
...
/* console.log(
convertUserNameReplyedDBObjectToResponseObject(getUsernameReply Tweets).
);*/
...
await database.run(delete TweetQuery);
